...
for x in $(seq "$WORKSPACESPERMONITOR"); do
		echo "bind = SUPER, ${x}, exec, ${HOME}/.local/share/scripts/workspaces.sh ${x} ${WORKSPACESPERMONITOR}";
done
echo ""
for x in $(seq "$WORKSPACESPERMONITOR"); do
		echo "bind = SUPER_SHIFT, ${x}, exec, ${HOME}/.local/share/scripts/workspaces-move.sh ${x} ${WORKSPACESPERMONITOR}";
done
echo ""
